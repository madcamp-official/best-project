...
class RoomManager {
    ...
    /**
     * 새 방 생성(자동 id 부여). private=true면 초대 코드를 발급하고(친구 초대용) 로비 공개 목록에서
     * 빠진다(RoomBroadcaster.roomListMessage) — 코드를 아는 사람만 /lobby/joinByCode로 입장 가능.
     */
    fun create(name: String, private_: Boolean = false): Room {
        val id = "r${seq.incrementAndGet()}"
        return Room(id, name).also {
            it.private = private_
