...
// 정적 CONFIG는 지도(mapId)마다 달라질 수 없는데, 시군구처럼 동보다 훨씬 큰 셀을 쓰는 지도는
// 미사일 반경·공수 사거리 같은 거리 기반 상수를 법정동 기준값 그대로 쓰면 의미가 없어진다
// (MapCatalog.applyProfile 참조) — 그래서 서버가 WELCOME.config로 지도별 오버라이드가
