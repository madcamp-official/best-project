...
export interface Connection {
  ...
  listRooms(): void;
  // 방 생성(생성 즉시 입장). onRoomJoined로 응답. isPrivate=true면 로비 목록에 안 뜨고
  // 서버가 초대 코드를 발급한다(onRoomJoined의 joinCode로 옴 — 친구에게 공유).
  ...
  // 공세 목표 지정/해제. index = 적·중립 admIndex, -1이면 해제. 이후 그 목표를 향해 전선이 자동 전진.
