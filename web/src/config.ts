...
export const CONFIG = {
  ...
  // 전술핵 사일로 — 울릉도(울릉읍)·제주도(오등동, 한라산 북사면)에 고정. 그 동을 소유한
  // 플레이어는 일반 미사일의 NUKE_RADIUS_MULT배 반경 미사일을 쿨다운마다 발사할 수 있다.
  // 일반 미사일이 단일 표적으로 바뀐 뒤로, 전술핵은 같은 반경(×1)이되 원에 걸친 여러 동을 한 번에 타격하는 광역 무기 역할을 한다.
  // 두 섬은 본토와 인접이 끊긴 컴포넌트라, 사일로 섬을 오가는 공수는 사거리 제한을 받지 않는다.
  NUKE_SILO_CODES: ["47940250", "50110134"], // [울릉읍(독도 포함), 오등동(제주·한라산)] — 클라·서버 공유
  NUKE_RADIUS_MULT: 1, // 전술핵 반경 = MISSILE_RADIUS_DEG × 1 (예전 미사일 범위). 광역(원 안 여러 동) 타격.
